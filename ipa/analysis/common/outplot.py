...
def vis_actin_surround_actin_dist(dist, file_idx, actin_num, point_num, ):
    ...
    # sns.kdeplot(datdist, shade=True)
...
def actin_to_vesicle_dist_angle_pair_plot(dist_angle_pairs , dataid, actin_num, point_num):

    actin_dist_lst = dist_angle_pairs['Distance']
    actin_angle_lst = dist_angle_pairs['Angle']
    

    max_ = np.max(actin_dist_lst)
    min_ = np.min(actin_dist_lst)
    points_x = actin_dist_lst
    points_y = actin_angle_lst

    # print(np.min(points_x), np.max(points_x))
    # print(np.min(points_y), np.max(points_y))

    plt.scatter(points_x,points_y)
    plt.xlim([int(np.floor(min_/100)*100),np.ceil(max_/100)*100])
    plt.ylim([0,90])
    plt.show()
    plt.close()


    if len(actin_dist_lst)>300:    
        bins_ = [50,50]
    elif len(actin_dist_lst)>100:
        bins_ = [50,50]
    else:
        bins_ = [10, 10]
    ...
    #     if points_y[ii]<50:
    #         # print(11)
    #         points_x_new.append(points_x[ii])
    #         points_y_new.append(points_y[ii])

    # print(len(points_x_new),len(points_x))
    ...
    fig = plt.figure(figsize=(8,6), dpi=1200)
    hist, xedges, yedges = np.histogram2d(points_x, points_y, bins=bins_, range =[[int(np.floor(min_/100)*100), np.ceil(max_/100)*100],[0,90]], )
    # hist, xedges, yedges = np.histogram2d(points_x, points_y, bins=bins_, range =[[min_, max_],[0,90]], )
    
    hist = np.rot90(hist)  # exchange x&y
    hist = hist[::-1,::]  # match final plot
    ...
    grid_x, grid_y = np.mgrid[0:hist.shape[0]:1000j, 0:hist.shape[1]:990j]
    ...
    # print(int(np.floor(min_/100)*100), int(np.ceil(max_/100)*100), (int(np.ceil(max_/100))-int(np.floor(min_/100)))*10*2)
    plt.title(f'{dataid}  actin to vesicle distance angle pair distribution \n actin num {actin_num}, points num {point_num}')
    # plt.xticks(np.arange(0,1000,200),np.arange(int(np.floor(min_/100)*100), int(np.ceil(max_/100)*100), (int(np.ceil(max_/100)-int(np.floor(min_/100)*100))*10*2)))
    plt.xticks(np.arange(0,1000,200), np.arange(int(np.floor(min_/100)*100), int(np.ceil(max_/100)*100), (int(np.ceil(max_/100))-int(np.floor(min_/100)))*10*2 ))
...
def actin_to_vesicle_dist_angle_pair_condition_plot(dist_angle_pairs ):
...
def dist2mem_condition_plot2(dist, file_idx, validpath, bins = 100, x_lim = 1200 ):
    ...
    sns.distplot(dist, kde=True, bins=bins,) #rug=True, bins=bins, )
    # sns.kdeplot(datdist, shade=True)
