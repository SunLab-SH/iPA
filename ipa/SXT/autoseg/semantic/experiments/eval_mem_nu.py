...
class cellmapping_test():
    def __init__(self, type='imageall_x'):
        ...
        self.checkpoint_dir = 'results/{}/best.pth'.format(args.exp)
    ...
    def test(self):
        ...
        for imagefile in self.test_idx:
            ...
            # for ii, samp in enumerate(test_loader):
            #     print(samp["image"])

            # print(osp.join(self.image_root_dir, imagefile))
            self.images_list = self.get_path(osp.join(self.image_root_dir, imagefile))
            # print('line146',self.images_list )
            self.model.eval()
    ...
    def step(self, dataloader, imagefile=None):
        ...
        for idx, sample in enumerate(dataloader):
            ...
            for i in range(images.shape[0]):
                ...
                temp_img = images[i].cpu().numpy()
                temp_img = temp_img.reshape(temp_img.shape[1],temp_img.shape[2])
                temp_pred = pred
