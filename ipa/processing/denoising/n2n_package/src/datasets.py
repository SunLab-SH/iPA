...
from .utils_n2n import load_hdr_as_tensor
...
class NoisyDataset(AbstractDataset):
    ...
    def __init__(self, root_dir, redux, crop_size, clean_targets=False,
        ...
        self.imgs = [f for f in os.listdir(root_dir) if os.path.isfile(os.path.join(root_dir, f)) and f.lower().endswith(valid_extensions) and not f.startswith('.')]
        if redux:
    ...
    def _add_noise(self, img):
        ...
        # Normal distribution (default)
            ...
            noise = np.random.normal(0, std, np.array(img).shape)
