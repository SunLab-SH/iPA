...
import inspect
# print(f"DEBUG: UNet loaded from {inspect.getfile(UNet)}")
# print(f"DEBUG: UNet init signature: {inspect.signature(UNet.__init__)}")
...
class Noise2Noise(object):
    ...
    def _compile(self):
        ...
        if self.p.noise_type == 'mc':
            self.is_mc = True
            self.model = UNetN2N(in_channels=9, out_channels=3)
        else:
            self.is_mc = False
            # SXT data is 1 channel (grayscale)
            self.model = UNetN2N(in_channels=1, out_channels=1)
    ...
    def test(self, test_loader, show):
        ...
        denoised_dir = os.path.dirname(self.p.data_dir)
