...
def load_3d_label(data_id, dataset_info=None):
    """加载3D标签文件，支持数据集别名映射"""
    # 检查是否有别名映射
    ...
    label_path = rf"D:\Gitspace\ipa_full\iPA\data\sxt_images\label_slice\{actual_data_id}\combine_{actual_data_id}_label.tif"
    
    if not os.path.exists(label_path):
        print(f"Warning: 3D label file not found: {label_path}")
        return None
    
    try:
        # 加载3D标签文件
        label_3d = tifffile.imread(label_path)
        print(f"    Loaded 3D label: {label_3d.shape}")
        return label_3d
    except Exception as e:
        print(f"    Error loading 3D label: {e}")
        return None
...
def get_mask_from_3d_label(label_3d, slice_mapping, filename, data_id, dataset_info):
    """从3D标签中提取对应切片的2D mask"""
    ...
    if slice_index >= label_3d.shape[0]:
        print(f"    Warning: Slice index {slice_index} out of range for 3D label shape {label_3d.shape}")
        return None
    ...
    if data_id in need_flip:
        ...
        print(f"    Applied vertical flip to mask for dataset {data_id}")
    
    print(f"    Extracted 2D mask from slice {slice_index}, shape: {mask_2d.shape}")
    
    return mask_2d


def get_image_mask_pairs(data_id):
    """获取指定data_id下的图像文件列表，使用3D标签生成mask"""
    ...
    for img_file in image_files:
        ...
        pairs.append((img_path, None, img_file))  # mask_path暂时为None，稍后从3D标签生成
    
    return pairs


def split_image_with_3d_mask(img_path, label_3d, slice_mapping, data_id, filename, cfg):
    """使用3D标签切分单个图像"""
    # 读取图像
    ...
    # 从3D标签生成2D mask
    mask = get_mask_from_3d_label(label_3d, slice_mapping, filename, data_id, dataset_info)
    
    if mask is None:
        print(f"    Warning: Could not generate mask for {filename}")
        return []
    ...
    if mask.shape != (raw_h, raw_w):
        print(f"    Warning: Mask shape {mask.shape} doesn't match image shape {(raw_h, raw_w)}")
        # 尝试调整mask尺寸
    ...
    for i, x in enumerate(x_points):
        for j, y in enumerate(y_points):
            ...
            print(f"    Mask area: {mask_area}, threshold: {cfg.min_mask_area}")
            
            if cfg.min_mask_area > 0:
                if mask_area < cfg.min_mask_area:
                    print(f"    Skipped due to small mask area")
                    continue
            
            split_info = {
                'original_image': img_path,
                'original_mask': f"3D_label_slice_{extract_slice_number(filename)}",
                'split_position': (x, y),
...
def split_image_and_mask(img_path, mask_path, cfg):
    """切分单个图像和对应的mask"""
    # 读取图像和mask
    img = cv.imread(img_path)
    ...
    if img is None or mask is None:
        print(f"Error: Cannot read {img_path} or {mask_path}")
        return []
...
def process_data_id(data_id, cfg):
    ...
    # 加载3D标签 - 传递dataset_info来处理别名映射
    label_3d = load_3d_label(data_id, dataset_info)
    
    if label_3d is None:
        print(f"Cannot load 3D label for {data_id}, skipping...")
        return [], [], []
    ...
    for img_path, mask_path, img_filename in pairs:
        ...
        # 使用3D标签切分图像
        split_results = split_image_with_3d_mask(img_path, label_3d, slice_mapping, data_id, img_filename, cfg)
...
def process_all_data(cfg):
    ...
    # 保存COCO格式标注文件
    coco_path = os.path.join(cfg.output_dir, 'split_instances.json')
    ...
    mapping_path = os.path.join(cfg.output_dir, 'split_mapping.json')
    with open(mapping_path, 'w', encoding='utf-8') as f:
        json.dump(split_mapping, f, indent=2, ensure_ascii=False)
    
    # 保存处理摘要
    summary_path = os.path.join(cfg.output_dir, 'split_summary.json')
...
def show_config(cfg):
    """显示配置信息"""
    print("=" * 50)
    print("SXT Image Split Configuration")
    print("=" * 50)
...
def init():
    """初始化参数"""
    parser = arg.ArgumentParser(description='Split SXT images and corresponding masks')
    
    # 基本参数
    parser.add_argument('-o', '--output', dest='output_dir', 
                       default=r'D:\Gitspace\ipa_full\iPA\data\sxt_images\split_img',
...
if __name__ == "__main__":
    ...
    print(f"\nAll done! Check results in: {cfg.output_dir}")