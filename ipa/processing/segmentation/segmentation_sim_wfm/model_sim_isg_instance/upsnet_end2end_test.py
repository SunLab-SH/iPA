...
def im_post(boxes_all, masks_all, scores, pred_boxes, pred_masks, cls_inds, num_classes, im_info):
    ...
    # print(f'confirm impost position A')
...
def upsnet_test():
    ...
    if args.eval_only:
        ...
        if config.network.has_rcnn:
            print(f'len all_boxes: {len(results["all_boxes"])}  all_masks: {len(results["all_masks"])}')
            print(f'len boxes-1 {len(results["all_boxes"][0])}  boxes-2: {len(results["all_boxes"][1])}')
            print(f'len masks-1 {len(results["all_masks"][0])}  masks-2: {len(results["all_masks"][1])}')
    ...
    #     print(f'\rBatch {i_iter}/{len(test_dataset)}  {round(i_iter/len(test_dataset)*100, 2)}%        ', end='')
    #     data_timer.tic()
    #     batch = []
    #     labels = []
    #     for gpu_id in gpus:
    ...
    #            }
    #
    # with open(os.path.join(final_output_path, 'results', 'results_list-pure.pkl'), 'wb') as f:
    #     pickle.dump(results, f, protocol=2)
    # print(f'\nFinal output path: {final_output_path}')
    # print(f'Save accomplished!')

    # TODO: 没有用GT筛选的情况下这样的evaluation无效，此参数为手动修改
    # exit()
    
    # with open('./output/upsnet/coco/upsnet_resnet50_ihuman_4gpu_scratch/test-ihuman-sst/results/results_list.pkl', 'rb') as f:
    #     results = pickle.load(f)
    # all_boxes = results['all_boxes']
    # all_masks = results['all_masks']

    # TODO: Merge Evaluation先测试segmentation的evaluation
    all_boxes = []
    all_masks = []

    # proposals = results['all_boxes']
