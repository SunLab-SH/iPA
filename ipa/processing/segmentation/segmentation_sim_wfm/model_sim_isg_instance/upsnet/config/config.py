...
config.output_path = r'D:\Gitspace\ipa_full\iPA\ipa\processing\segmentation\segmentation_sxt\model_sxt_isg_instance\output'
config.model_prefix = 'upsnet_'
config.symbol = 'resnet50_upsnet'
config.gpus = '0'

# network related params
config.network = edict()
config.network.pretrained = ''
config.network.use_pretrained = False
...
# 冻结相关配置
config.network.fpn_freeze = False
config.network.rpn_freeze = False 
config.network.backbone_freeze = False
config.network.rcnn_freeze = False
config.network.mask_freeze = False
...
config.dataset.dataset = 'CocoDataset'
config.dataset.image_set = 'train'
config.dataset.test_image_set = 'val'
config.dataset.root_path = ''
config.dataset.dataset_path = ''
config.dataset.num_classes = 81  # COCO默认类别数
...
config.train.batch_size = 2  # 适合单GPU的batch size

# 添加缺失的训练参数
config.train.momentum = 0.9
config.train.wd = 0.0001
config.train.lr = 0.001
config.train.max_iteration = 10000
config.train.display_iter = 100
config.train.snapshot_step = 1000
config.train.warmup_iteration = 500
config.train.decay_iteration = [7000, 9000]

# panoptic head related param
config.train.fcn_loss_weight = 1.0
config.train.fcn_with_roi_loss = False
config.train.panoptic_loss_weight = 1.0
config.train.bbox_loss_weight = 1.0
