...
def soft_nms(
    ...
    # Fallback to standard NMS if cython version is not available
    try:
        import cython_nms
        ...
        # Python fallback - simple hard NMS
        return _py_soft_nms(dets, sigma, overlap_thresh, score_thresh, method)


def _py_soft_nms(dets, sigma=0.5, overlap_thresh=0.3, score_thresh=0.001, method='linear'):
    """Python implementation of soft NMS as fallback."""
    if dets.shape[0] == 0:
        return dets, []
    
    # Simple hard NMS fallback
    x1 = dets[:, 0]
    y1 = dets[:, 1]
    x2 = dets[:, 2]
    y2 = dets[:, 3]
