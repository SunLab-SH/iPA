...
class resnet_upsnet(resnet_rcnn):
    ...
    def forward(self, data, label=None, index=None, anno_info=None):
        #''' hcy
        if config.save_rois:
        ...
        if label is not None:
            ...
            rois_, scores = self.pyramid_proposal(rpn_cls_prob, rpn_bbox_pred, data['im_info'])
            rois, cls_label, bbox_target, bbox_inside_weight, bbox_outside_weight, mask_rois, mask_target, roi_has_mask, nongt_inds = self.proposal_target(rois_, label['roidb'], data['im_info'])
        else:
            self.pyramid_proposal = PyramidProposal(feat_stride=config.network.rpn_feat_stride, scales=config.network.anchor_scales,
            ...
            rois, scores = self.pyramid_proposal(rpn_cls_prob, rpn_bbox_pred, data['im_info'])

        if config.save_rois and label is None: # hcy: save roi coordinates with txt format. Note: return directly
            proposals = rois[:, 1:].cpu().numpy().astype(np.int)
            preds = scores.cpu().numpy()
            
            # print(f'Image_id: {image_id}  image_path: {image_name}')
            save_dir = '/root/ups_output/rpn_inference_data/'
            with open(save_dir+str(image_id)+'_rpn_data.txt', 'w', encoding='utf-8') as f:
                f.write(f'id {image_id} height {int(data["im_info"][0][0])} width {int(data["im_info"][0][1])} scale {float(data["im_info"][0][2])}\n')
                f.write(f'path {image_name}\n')
                f.write(f'start_gt {len(gt_bboxes)}\n')
                for gt in gt_bboxes:
                    a,b,c,d = gt
                    f.write(f'{int(a)} {int(b)} {int(c)} {int(d)}\n')
                f.write(f'start_roi {len(proposals)}\n')
                for k in range(proposals.shape[0]):
                    f.write(f'{proposals[k, 0]} {proposals[k, 1]} {proposals[k, 2]} {proposals[k, 3]} {preds[k]}\n')
                f.write(f'end_roi')
            return None
        ...
        if label is not None:
            ...
            output = {
                ...
                'cls_loss': cls_loss.unsqueeze(0) if config.network.has_rcnn else None,
                'bbox_loss': bbox_loss.unsqueeze(0) if config.network.has_rcnn else None,
                'mask_loss': mask_loss.unsqueeze(0) if config.network.has_rcnn else None,
                'fcn_loss': fcn_loss.unsqueeze(0) if config.network.has_fcn_head else None,
                'panoptic_loss': panoptic_loss.unsqueeze(0) if config.network.has_panoptic_head else None,
                'rcnn_accuracy': rcnn_acc.unsqueeze(0) if config.network.has_rcnn else None,
                'panoptic_accuracy': panoptic_acc.unsqueeze(0) if config.network.has_panoptic_head else None,
            }
            if config.network.has_fcn_head and config.train.fcn_with_roi_loss:
