...
class coco(BaseDataset):
    ...
    def __init__(self, image_sets, flip=False, proposal_files=None, phase='train', result_path=''):
        ...
        image_dirs = {
            ...
            'train-ihuman-sst': os.path.join(config.dataset.dataset_path, 'images', 'train'),
            'val-ihuman-sst': os.path.join(config.dataset.dataset_path, 'images', 'val'),
            'test-ihuman-sst': os.path.join(config.dataset.dataset_path, 'images', 'test')
        }
        ...
        anno_files = {
            ...
            'train-ihuman-sst': 'instances_train2014.json',
            'val-ihuman-sst': 'instances_val2014.json',
            'test-ihuman-sst': 'instances_test2014.json'
        }
        ...
        if phase == 'train' and len(image_sets) > 1:
            ...
            print(f'image_dir: {image_dirs[image_sets[0]]}')
            print(f'anno_file: {os.path.join(config.dataset.dataset_path, "annotations", anno_files[image_sets[0]])}')
        ...
        self.num_classes = 2 # 81 hcy

        self.hcy_index = -1 # hcy

    def __len__(self):
        return len(self.roidb)

    def get_anno_info(self):
        assert self.hcy_index != -1
        res = self.roidb[self.hcy_index]
        self.hcy_index = -1
        return res

    def __getitem__(self, index):
        self.hcy_index = index # hcy
        blob = defaultdict(list)
        im_blob, im_scales = self.get_image_blob([self.roidb[index]])
        # print(f'information.roidb: {self.roidb[index]}') # hcy
        now_info = self.roidb[index] # hcy
        ...
        if config.network.has_rpn:
            if self.phase != 'test':
                ...
                data = {'data': im_blob,
                        ...
                        'bboxes': now_info['boxes'],  # hcy
                        'image_id': now_info['id'],
                        'image_name': now_info['image']}  # hcy
                label = None
        else:
    ...
    def evaluate_masks(self, all_boxes, all_segms, output_dir,):
        # TODO: 直接加载我处理好的segmentation文件
        # res_file = os.path.join(
        #     output_dir, 'segmentations_' + self.dataset.name + '_results-pure.json'
        # )
        # results = []
        ...
        print('load segmentation file by hcy')
        res_file = '/p300/ihuman/UPSNet/data/ihuman_sst/annotations/3d-pure_evaluation_origin_test_784.json'
        print(res_file)
