...
# Read data file
basal_fp = []
for cellnumber in seqs[:7]:   # 2.8 mM
    print('b:'+cellnumber)
    k1 = np.loadtxt(f'../../05_rdf/results/{cellnumber}_t8_velocity_dist_ne-pm.xvg')
    basal_fp.append(k1)

glu_5_fp = []
for cellnumber in seqs[7:13]:   # 16.7 mM - 5 min
    print('5:'+cellnumber)
    k1 = np.loadtxt(f'../../05_rdf/results/{cellnumber}_t8_velocity_dist_ne-pm.xvg')
    glu_5_fp.append(k1)

glu_30_fp = []
for cellnumber in seqs[13:]:   # 16.7 mM - 30 min
    print('30:'+cellnumber)
    k1 = np.loadtxt(f'../../05_rdf/results/{cellnumber}_t8_velocity_dist_ne-pm.xvg')
    glu_30_fp.append(k1)   
...
Rne = 34939
Risg = 1289
dr = (Rcell - Rne)/N_shell

xdata = np.linspace(1, N_shell, len(k1))*dr
...
fig.savefig(F'../../05_rdf/results/vel_rdf_ne_pm.png', transparent=True, dpi=300)
...
for cellnumber in seqs[:7]:   # 2.8 mM
    print('b:'+cellnumber)
    k1 = np.loadtxt(f'../../04_versicle/{cellnumber}_PM_NE_mask_t8_coord_voxel_3d_velocity.xvg', dtype=float)
    basal_fp.append(k1)

basal_fp = np.array(basal_fp, dtype=float)
vel_basal = []
for i in range(7):
    vel = np.sqrt((basal_fp[i][:, 0] - basal_fp[i][:, 3])^2 + (basal_fp[i][:, 1] - basal_fp[i][:, 4])^2 + (basal_fp[i][:, 2] - basal_fp[i][:, 5])^2)
    vel_basal.append(vel)
# basal_fp[0]
