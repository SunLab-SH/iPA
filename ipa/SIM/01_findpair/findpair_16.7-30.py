...
os.getcwd() 
file_list = glob.glob(f'masks\\16.7-30*.npy')
for file in file_list:
    mask_t = np.load(file) # T, z, C(PM/NE), x, y
    seq = file[6:-4]
    print(seq)
    # raw_mask = np.load('masks\\' + seq + '.npy') # T, z, C(PM/NE), x, y
    for t in range(len(mask_t)):
        k1 = np.loadtxt(f'01_index\\{seq}_ne_index_t{t}.xvg')
        k2 = np.loadtxt(f'01_index\\{seq}_pm_index_t{t}.xvg')
        ...
        pair_out=open(f'02_findpair\\{seq}_pair_ne-pm_t{t}.xvg', 'w') # output
        t1 = 0
        t2 = int(len(pm)/8.0)
        print(len(pm))
        print(t2)
