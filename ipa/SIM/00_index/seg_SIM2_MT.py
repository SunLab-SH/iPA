...
file_list=[]
with open('../file_list_MT_ex4.txt','r') as f:
	for line in f:
...
####################### save for he junjie ##########################
tif_dir = 'I:\\Fluorescence\\SIM\\c\\Singlecolor\\'
dir = 'I:\\Bing\\fluorescence\\3D\\06_stat\\'
...
for file_name in file_list:
	...
	correct_MT = np.full((64, 1600, 1600), 0)
	correct_MT[:PM_mask.shape[0], corr_x_low_bnd:corr_x_high_bnd, corr_y_low_bnd:corr_y_high_bnd] \
		= img_MT[:, x_low_bnd:x_high_bnd, y_low_bnd:y_high_bnd]
	MT_saved = []
	for i in range(64):
		MT_bin2 = resize(correct_MT[i].reshape(-1, 1600, 1600))
		MT_saved.append(MT_bin2)
	MT_saved = np.array(MT_saved, dtype=np.int16).reshape(64,800,800)
	...
	with mrcfile.new(f'{dir}{file_name}_volumn_bin2_MT.mrc', overwrite=True) as mrc:
		mrc.set_data(MT_saved*pm_total)
