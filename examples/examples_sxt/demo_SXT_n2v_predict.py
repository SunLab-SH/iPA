"""
Demo: N2V Denoising Prediction on Real SXT Data

This example demonstrates how to use a trained N2V model to denoise real SXT data.
"""
...
def main():
    print("=" * 70)
    print("N2V Denoising Prediction (Real SXT Data)")
    print("=" * 70)
    ...
    model_dir = os.path.join(PROJECT_ROOT, 'ipa', 'processing', 'denoising', 'models', 'n2v_3D')
    model_path = os.path.join(model_dir, 'weights_best.h5')
    
    if not os.path.exists(model_path):
        print(f"Error: Trained model not found at {model_path}. Please run demo_SXT_n2v_train.py first.")
        return
    ...
    axes[1].imshow(denoised_data[slice_idx], cmap='gray'); axes[1].set_title('Denoised (N2V)')
    axes[2].imshow(np.abs(noisy_data[slice_idx] - denoised_data[slice_idx]), cmap='hot'); axes[2].set_title('Difference')
    for ax in axes: ax.axis('off')
    
    out_path = os.path.join(ipa_root, 'results', 'demo_n2v_predict_sxt.png')
